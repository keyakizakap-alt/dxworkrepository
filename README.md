...
配置後、`claude` を起動し直すと `/skills` に `work-directives` と `ai-product-playbook` が並ぶ。

**Claude Code on the web の場合**は `~/.claude/skills/` が使えない。セッションにこのリポジトリを
追加すると読み込まれる（各アプリの `CLAUDE.md` にその手順を書いてある）。
